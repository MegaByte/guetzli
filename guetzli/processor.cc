...
class Processor {
 ...
 private:
  void SelectFrequencyMasking(const JPEGData& jpg, OutputImage* img,
                              ...
                              bool stop_early);
  void ComputeBlockZeroingOrder(const coeff_t block[kBlockSize],
                                const coeff_t orig_block[kBlockSize],
                                const int block_x, const int block_y,
                                const int factor_x, const int factor_y,
                                const uint8_t comp_mask, OutputImage* img,
  ...
  QuantData TryQuantMatrix(const JPEGData& jpg_in, const float target_mul,
                           int q[3][kDCTBlockSize]);
  void MaybeOutput(const JPEGData& jpeg, const std::string& encoded_jpg);
...
void Processor::OutputJpeg(const JPEGData& jpg, std::string* out) {
  out->clear();
...
void Processor::MaybeOutput(const JPEGData& jpeg,
                            const std::string& encoded_jpg) {
...
QuantData Processor::TryQuantMatrix(const JPEGData& jpg_in,
  ...
  GUETZLI_LOG(stats_, "Iter %2d: %s quantization matrix:\n",
              stats_->counters[kNumItersCnt] + 1, img.FrameTypeStr().c_str());
  ...
  MaybeOutput(jpg_out, encoded_jpg);
  return data;
}
...
bool Processor::SelectQuantMatrix(const JPEGData& jpg_in, const bool downsample,
  ...
  for (;;) {
    ...
    QuantData data = TryQuantMatrix(jpg_in, target_mul_high, q_next);
    qgen.Add(data);
...
void Processor::ComputeBlockZeroingOrder(const coeff_t block[kBlockSize],
                                         const coeff_t orig_block[kBlockSize],
                                         const int block_x, const int block_y,
                                         const int factor_x, const int factor_y,
  ...
  static const uint8_t oldCsf[kDCTBlockSize] = {
      10, 10, 20, 40, 60, 70, 80, 90, 10, 20, 30, 60, 70, 80, 90, 90,
      20, 30, 60, 70, 80, 90, 90, 90, 40, 60, 70, 80, 90, 90, 90, 90,
      60, 70, 80, 90, 90, 90, 90, 90, 70, 80, 90, 90, 90, 90, 90, 90,
...
#include "guetzli/order.inc"
  ...
  while (!input_order.empty()) {
    ...
    for (int i = 0; i < std::min<size_t>(params_.zeroing_greedy_lookahead,
      ...
      for (int c = 0; c < 3; ++c) {
        if (comp_mask & (1 << c)) {
          img->component(c).SetCoeffBlock(block_x, block_y,
                                          &candidate_block[c * kDCTBlockSize]);
        }
      }
      float max_err = 0;
    ...
    for (int c = 0; c < 3; ++c) {
      if (comp_mask & (1 << c)) {
        img->component(c).SetCoeffBlock(block_x, block_y,
                                        &processed_block[c * kDCTBlockSize]);
      }
    }
  }
...
void UpdateACHistogram(const int weight, const coeff_t* coeffs, const int* q,
...
size_t ComputeEntropyCodes(const std::vector<JpegHistogram>& histograms,
  ...
  std::vector<uint8_t> clustered_depths(histograms.size() *
                                        JpegHistogram::kSize);
...
size_t EntropyCodedDataSize(const std::vector<JpegHistogram>& histograms,
  ...
  for (int i = 0; i < histograms.size(); ++i) {
    numbits +=
        HistogramEntropyCost(histograms[i], &depths[i * JpegHistogram::kSize]);
  }
  return (numbits + 7) / 8;
}
...
void Processor::SelectFrequencyMasking(const JPEGData& jpg, OutputImage* img,
  ...
  for (int block_y = 0, block_ix = 0; block_y < block_height; ++block_y) {
    for (int block_x = 0; block_x < block_width; ++block_x, ++block_ix) {
      ...
      ComputeBlockZeroingOrder(block, orig_block, block_x, block_y, factor_x,
                               factor_y, comp_mask, img, &orders[block_ix]);
    }
  }
  ...
  for (int direction : {1, -1}) {
    for (;;) {
          ...
          for (int block_x = 0; block_x < block_width; ++block_x, ++block_ix) {
            ...
            if (direction > 0) {
              for (int i = last_index; i < order.size(); ++i) {
                float val =
                    ((order[i].block_err - max_err) / block_weight[block_ix]);
              ...
              for (int i = last_index - 1; i >= 0; --i) {
                float val =
                    ((max_err - order[i].block_err) / block_weight[block_ix]);
                global_order.push_back(std::make_pair(block_ix, val));
              }
      ...
      std::sort(
          global_order.begin(), global_order.end(),
          [](const std::pair<int, float>& a, const std::pair<int, float>& b) {
            return a.second < b.second;
          });
      ...
      if (first_up_iter) {
        ...
        auto it = std::partition_point(
            global_order.begin(), global_order.end(),
            [=](const std::pair<int, float>& a) { return a.second < limit; });
        min_coeffs_to_change =
            std::max<int>(min_coeffs_to_change, it - global_order.begin());
        first_up_iter = false;
      }
      ...
      for (int i = 0; i < global_order.size(); ++i) {
        ...
        const int newval =
            direction > 0
                ? 0
                : Quantize(comp.coeffs[jpg_block_ix * kDCTBlockSize + k],
                           quant[k]);
      ...
      GUETZLI_LOG(stats_,
                  ...
                  global_order.size(), changed_blocks.size(), blocks_to_change,
                  num_blocks, val_threshold, encoded_jpg.size(),
...
bool Processor::ProcessJpegData(const Params& params, const JPEGData& jpg_in,
  ...
  // The jpg quality to Butteraugli scores is calculated in quality.cc.
...
bool Process(const Params& params, const JPEGData& jpeg_in,
             JPEGData* jpeg_out) {
  std::vector<uint8_t> rgb = DecodeJpegToRGB(jpeg_in);
  GuetzliOutput out;
  ProcessStats dummy_stats;
  ButteraugliComparator comparator(jpeg_in.width, jpeg_in.height, rgb,
                                   params.butteraugli_target);
  bool ok = ProcessJpegData(params, jpeg_in, &comparator, &out, &dummy_stats);
  *jpeg_out = out.jpeg;
  return ok;
}

bool Process(const Params& params, const std::string& data,
             std::string* jpg_out) {
  JPEGData jpg;
  ...
  ButteraugliComparator comparator(jpg.width, jpg.height, rgb,
                                   params.butteraugli_target);
  bool ok = ProcessJpegData(params, jpg, &comparator, &out, &dummy_stats);
  *jpg_out = out.jpeg_data;
  return ok;
}

bool Process(const Params& params, const std::vector<uint8_t>& rgb, int w,
             int h, std::string* jpg_out) {
  JPEGData jpg;
  if (!EncodeRGBToJpeg(rgb, w, h, &jpg)) {
    fprintf(stderr, "Could not create jpg data from rgb pixels\n");
    return false;
